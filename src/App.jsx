...
async function fetchTx() {
  ...
  return data.map(r => ({ id: r.id, userId: r.user_id, type: r.type, category: r.category, emoji: r.emoji, amount: parseFloat(r.amount), note: r.note || "", date: r.date, recurring: r.recurring, sg: r.split_group_id, stotal: r.split_total ? parseFloat(r.split_total) : null }));
}
async function insertTx(tx) {
  const { error } = await supabase.from("transactions").insert({ user_id: tx.userId, type: tx.type, category: tx.category, emoji: tx.emoji, amount: tx.amount, note: tx.note, date: tx.date, recurring: tx.recurring, split_group_id: tx.sg || null, split_total: tx.stotal || null });
...
async function getS(key, fb) { const { data } = await supabase.from("settings").select("value").eq("key", key).single(); return data ? (typeof data.value === 'string' ? JSON.parse(data.value) : data.value) : fb; }
async function setS(key, val) { await supabase.from("settings").upsert({ key, value: JSON.stringify(val), updated_at: new Date().toISOString() }); }
...
export default function App() {
  ...
  // Load
  useEffect(() => { (async () => {
    const [t, b, d, th, cc, sh] = await Promise.all([
    ...
    setDark(d === true || d === "true"); setTheme(th || "sunset");
    setCustomCats(cc || { income: [], expense: [] });
    setSettleHistory(sh || []);
    setReady(true);
  })(); }, []);
  ...
  useEffect(() => {
    const ch = supabase.channel("bb-rt")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "settings" }, async () => {
        setBudget(parseFloat(await getS("monthly_budget", 1500)) || 1500);
        setDark(v => { const nv = getS("dark_mode", false); return nv; });
      }).subscribe();
  ...
  const pranavPaidSplit = txs.filter(x => x.date.startsWith(month) && x.sg && x.userId === "pranav").reduce((s, x) => s + x.amount, 0);
  const keshaPaidSplit = txs.filter(x => x.date.startsWith(month) && x.sg && x.userId === "kesha").reduce((s, x) => s + x.amount, 0);
  const pranavTotal = uStats.find(u => u.id === "pranav")?.exp || 0;
  const keshaTotal = uStats.find(u => u.id === "kesha")?.exp || 0;
  const diff = pranavTotal - keshaTotal;
  // positive = pranav spent more, kesha owes; negative = kesha spent more, pranav owes
  ...
  const settleUp = async () => {
    const entry = { date: new Date().toISOString(), month: month, amount: Math.abs(diff), from: diff > 0 ? "kesha" : "pranav", to: diff > 0 ? "pranav" : "kesha" };
  ...
  const addTx = async () => {
    ...
    if (split === "split") {
      ...
      await insertTx({ ...base, userId: "pranav", amount: half, sg: gid, stotal: raw });
      await insertTx({ ...base, userId: "kesha", amount: half, sg: gid, stotal: raw });
      ...
      {view === "home" && (
        <div style={{ padding: "16px 14px 105px", animation: "fi 0.25s" }}>
                ...
                <div style={{ ...C, marginBottom: 10, animation: "su 0.47s", borderLeft: `3px solid #F59E0B`, padding: "13px 15px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <p style={{ fontSize: 10, color: c.sf, marginTop: 2 }}>
                        <strong style={{ color: diff > 0 ? USERS[1].color : USERS[0].color }}>{diff > 0 ? "Kesha" : "Pranav"}</strong> owes <strong style={{ color: diff > 0 ? USERS[0].color : USERS[1].color }}>{diff > 0 ? "Pranav" : "Kesha"}</strong> <strong style={{ color: c.tx }}>{fmt(Math.abs(diff) / 2)}</strong>
                      </p>
                    </div>
                    <button className="h" onClick={settleUp} style={{ ...B, background: "#F59E0B", color: "#fff", padding: "7px 12px", borderRadius: 10, fontSize: 10 }}>Mark Settled</button>
                  </div>
                </div>
              )}
            </>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 10, animation: "su 0.52s" }}>
            ...
            <button className="h" onClick={exportCSV} style={{ ...B, ...C, flex: 1, padding: "10px 8px", textAlign: "center", fontSize: 10.5 }}>📥 Export CSV</button>
          </div>
              ...
              <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: i < Math.min(mTx.length, 6) - 1 ? `1px solid ${c.bd}` : "none", animation: `su ${0.12 + i * 0.04}s ease` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 3 }}><p style={{ fontSize: 11.5, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.category}</p>
                    ...
                    {tx.recurring && <span style={{ fontSize: 7.5 }}>🔁</span>}</div>
