              ...
              <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: i < Math.min(mTx.length, 6) - 1 ? `1px solid ${c.bd}` : "none", animation: `su ${0.12 + i * 0.04}s ease` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 3 }}><p style={{ fontSize: 11.5, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.category}</p>
                    ...
                    {tx.receiptUrl && <span onClick={(e) => { e.stopPropagation(); setViewingReceipt(tx.receiptUrl); }} style={{ fontSize: 10, cursor: "pointer", background: c.pg, padding: "2px 6px", borderRadius: 5, color: c.p, fontWeight: 600, border: `1px solid ${c.p}30` }}>📷 Receipt</span>}</div>
            ...
            <div style={{ ...C, marginBottom: 12 }}>
              ...
              {!receiptPreview ? (
                ...
                <div style={{ position: "relative" }}>
                  <img src={receiptPreview} alt="Receipt" style={{ width: "100%", maxHeight: 200, objectFit: "cover", borderRadius: 10, border: `1px solid ${c.bd}` }} />
                  <button onClick={clearReceipt} style={{ position: "absolute", top: 6, right: 6, ...B, width: 28, height: 28, borderRadius: "50%", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
                  <p style={{ fontSize: 9.5, color: c.inc, marginTop: 6, textAlign: "center" }}>✅ Receipt attached</p>
                </div>
              )}
            </div>
          )}
          ...
          {mTx.length === 0 ? (<div style={{ textAlign: "center", padding: 30 }}><p style={{ fontSize: 28, marginBottom: 4 }}>🔍</p><p style={{ color: c.sf, fontSize: 11 }}>Nothing here</p></div>
            ...
            {mTx.map((tx, i) => { const u = USERS.find(u => u.id === tx.userId); return (
              <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: i < mTx.length - 1 ? `1px solid ${c.bd}` : "none", animation: `su ${0.08 + i * 0.03}s ease` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 2 }}><p style={{ fontSize: 11, fontWeight: 500 }}>{tx.category}</p>{tx.sg && <span style={{ fontSize: 7, background: c.ag, color: c.ac, padding: "0.5px 3px", borderRadius: 3.5, fontWeight: 600 }}>Split</span>}
                    {tx.receiptUrl && <span onClick={(e) => { e.stopPropagation(); setViewingReceipt(tx.receiptUrl); }} style={{ fontSize: 10, cursor: "pointer", background: c.pg, padding: "2px 6px", borderRadius: 5, color: c.p, fontWeight: 600, border: `1px solid ${c.p}30` }}>📷 Receipt</span>}
                  </div>
