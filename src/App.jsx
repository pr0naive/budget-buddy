...
async function fetchTx() {
  ...
  return data.map(r => ({ id: r.id, userId: r.user_id, type: r.type, category: r.category, emoji: r.emoji, amount: parseFloat(r.amount), note: r.note || "", date: r.date, recurring: r.recurring, sg: r.split_group_id, stotal: r.split_total ? parseFloat(r.split_total) : null, paidBy: r.paid_by || null }));
}
async function insertTx(tx) {
  const { error } = await supabase.from("transactions").insert({ user_id: tx.userId, type: tx.type, category: tx.category, emoji: tx.emoji, amount: tx.amount, note: tx.note, date: tx.date, recurring: tx.recurring, split_group_id: tx.sg || null, split_total: tx.stotal || null, paid_by: tx.paidBy || null });
...
export default function App() {
  ...
  // PIN functions
  const verifyPin = () => {
    if (pinInput === storedPin) { setLocked(false); setPinInput(""); setPinError(false); }
  ...
  const saveNewPin = async () => {
    ...
    await setS("app_pin", newPin);
    setStoredPin(newPin); setSettingPin(false); setNewPin(""); setConfirmPin("");
    setLocked(false);
    flash("PIN set! 🔐");
  };
  ...
  const addTx = async () => {
    ...
    if (split === "split") {
      ...
      // Both get half as their expense, but paidBy tracks who actually paid
          ...
          {[1,2,3,4,5,6,7,8,9,null,0,'⌫'].map((n, i) => (
            ...
            <button key={i} onClick={() => {
              if (n === '⌫') { setPinInput(p => p.slice(0, -1)); }
                ...
                setPinInput(next);
                if (next.length === 4) { setTimeout(() => { if (next === storedPin) { setLocked(false); setPinInput(""); } else { setPinError(true); setPinInput(""); setTimeout(() => setPinError(false), 1500); } }, 150); }
              }
