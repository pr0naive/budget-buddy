...
export default function App() {
  ...
  // Settle up calculation - based on ALL spending, whoever spent more is owed the difference / 2
  const pranavExp = uStats.find(u => u.id === "pranav")?.exp || 0;
  const keshaExp = uStats.find(u => u.id === "kesha")?.exp || 0;
  const totalSpent = pranavExp + keshaExp;
  const fairShare = totalSpent / 2;
  // If pranav spent more than fair share, kesha owes him the difference
  // If kesha spent more than fair share, pranav owes her the difference
  const settleAmount = Math.abs(pranavExp - fairShare);
  const whoOwes = pranavExp > keshaExp ? "kesha" : "pranav";
  const whoIsOwed = pranavExp > keshaExp ? "pranav" : "kesha";
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <p style={{ fontSize: 10, color: c.sf, marginTop: 2 }}>
                        <strong style={{ color: USERS.find(u => u.id === whoOwes)?.color }}>{USERS.find(u => u.id === whoOwes)?.name}</strong> owes <strong style={{ color: USERS.find(u => u.id === whoIsOwed)?.color }}>{USERS.find(u => u.id === whoIsOwed)?.name}</strong> <strong style={{ color: c.tx }}>{fmt(settleAmount)}</strong>
                      </p>
                    </div>
